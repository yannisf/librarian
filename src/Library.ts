...
export interface ILibrary {
    checkin(bookId: number, personId: number): void
    checkout(bookId: number, personId: number): void
}
...
export class Library implements ILibrary {
    ...
    checkin(bookId: number) {
        ...
        if (book && !book.available) {
            console.info("Returning book:", book)
            book.available = true
            book.borrower = null
        } else {
            console.warn("Book already available:", book)
        }
    }

    checkout(bookId: number, personId: number) {
        ...
        let book: Book|undefined = this.findBook(bookId)
        if (book && book.available) {
            console.info("Checking out book:", book.toString())
    ...
    private processCheckout(person: Person, book: Book) {
        ...
        return _.cloneDeep(_.omit(book, ["available", "borrower"]))
    }
